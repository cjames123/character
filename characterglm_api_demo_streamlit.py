...
from data_types import TextMsg, ImageMsg, TextMsgList, MsgList, filter_image_msg
...
with st.container():
    ...
    with col1:
        st.text_input(label="角色名", key="bot_name", on_change=lambda: st.session_state["meta"].
                      update(bot_name=st.session_state["bot_name"]), help="模型所扮演的角色的名字，不可以为空")
        st.text_area(label="角色人设", key="bot_info", on_change=lambda: st.session_state["meta"]
                     .update(bot_info=st.session_state["bot_info"]), help="角色的详细人设信息，不可以为空")
        
    with col2:
        st.text_input(label="用户名", value="用户", key="user_name", on_change=lambda: st.session_state["meta"]
                      .update(user_name=st.session_state["user_name"]), help="用户的名字，默认为用户")
        st.text_area(label="用户人设", value="", key="user_info", on_change=lambda: st.session_state["meta"]
                     .update(user_info=st.session_state["user_info"]), help="用户的详细人设信息，可以为空")
...
def draw_new_image():
    ...
    if not verify_meta():
        return
    # 过滤掉与role=image的消息
    text_messages = filter_image_msg(st.session_state["history"])
    if text_messages:
...
def start_chat():
    ...
    if not query:
        ...
        response_stream = get_characterglm_response(filter_image_msg(st.session_state["history"]), meta=st.session_state["meta"])
